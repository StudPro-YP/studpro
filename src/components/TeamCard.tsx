...
export const TeamCard = ({member}: TeamCardProps) => {
    return (
        <Card
            className="bg-white hover:bg-gray-100 rounded-lg py-6 m-8 flex flex-col gap-2 items-center w-64 border-none shadow-none">
            ...
            <h3 className="text-lg font-bold uppercase text-gray-900 dark:text-white mb-2">{member.name}</h3>
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">{member.role}</p>
            <div className="flex gap-6">
                {member.contactNo && (
                    <Link href={`tel:${member.contactNo}`} target="_blank" rel="noopener noreferrer">
                        <Phone size={20}/>
                    </Link>
                )}
                {member.email && (
